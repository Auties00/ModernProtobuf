...
public class ProtobufDeserializationVisitor extends ProtobufInstrumentationVisitor {
    ...
    private void createMessageDeserializer() {
        ...
        for(var property : message.properties()) {
            writer.println("        %s %s = %s;".formatted(property.type().fieldType(), property.name(), property.defaultValue()));
        }
    ...
    private void writeMapSerializer(ProtobufPropertyStub property, ProtobufPropertyType.MapType mapType) {
        ...
        writer.println("                        %s %s = null;".formatted(mapType.keyType().fieldType(), keyName));
        writer.println("                        %s %s = null;".formatted(mapType.valueType().fieldType(), valueName));
    ...
    private String getReadValue(String streamName, ProtobufPropertyType type, String readMethod) {
        ...
        var specName = getSpecName(type.implementationType());
        if(type.isEnum()) {
