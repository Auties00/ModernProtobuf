...
import it.auties.protobuf.serialization.jackson.exception.ProtobufException;
import it.auties.protobuf.serialization.jackson.exception.ProtobufSerializationException;
import it.auties.protobuf.base.ProtobufMessage;
import it.auties.protobuf.serialization.jackson.stream.ArrayOutputStream;
import it.auties.protobuf.serialization.jackson.model.ProtobufField;
...
class ProtobufGenerator extends GeneratorBase {
    ...
    public byte[] encode(Object object) {
        ...
        try {
            ...
            findFields(object.getClass())
                    ...
                    .map(field -> ProtobufField.ofWrite(object, field))
    ...
    private void encodeField(ArrayOutputStream output, ProtobufField field) {
        try {
            ...
            switch (field.type()) {
                case BOOL -> output.writeBool(field.index(), field.toValue());
                case STRING -> output.writeString(field.index(), field.toValue());
                case BYTES -> output.writeByteArray(field.index(), field.toValue());

                case FLOAT -> output.writeFixed32(field.index(), Float.floatToRawIntBits(field.toValue()));
                case DOUBLE -> output.writeFixed64(field.index(), Double.doubleToRawLongBits(field.toValue()));

                case INT32, SINT32 -> output.writeInt32(field.index(), field.toValue());
                case UINT32 -> output.writeUInt32(field.index(), field.toValue());
                case FIXED32, SFIXED32 -> output.writeFixed32(field.index(), field.toValue());

                case INT64, SINT64 -> output.writeInt64(field.index(), field.toValue());
                case UINT64 -> output.writeUInt64(field.index(), field.toValue());
                case FIXED64, SFIXED64 -> output.writeFixed64(field.index(), field.toValue());
